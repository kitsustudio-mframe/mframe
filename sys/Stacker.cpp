 ...
 * Static Method
 */
Stacker* Stacker::construct(void* pointer, int size) {
  void* stackPointer = pointer;
  pointer = Pointers::pointShift(pointer, sizeof(Stacker));
  size -= sizeof(Stacker);
  return new(stackPointer) Stacker(pointer, size);
}
/* ****************************************************************************
 * End of file
 */
